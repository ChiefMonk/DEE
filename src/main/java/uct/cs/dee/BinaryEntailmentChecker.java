...
public class BinaryEntailmentChecker 
{
    ...
    static Boolean checkEntailmentBinarySearch(PlBeliefSet[] rankedKB, PlFormula formula, int left, int right, PlFormula negationOfAntecedent) 
    {
        if (right >= left) 
        {
            ...
            if (classicalReasoner.query(combine(Arrays.copyOfRange(rankedKB, mid + 1, rankedKB.length)), negationOfAntecedent)) 
            {
        ...
        {
            if (classicalReasoner.query(combine(Arrays.copyOfRange(rankedKB, rankFromWhichToRemove + 1, rankedKB.length)), formula)) 
            {
                return true;
            } 
            else 
